...
double Image::change_scale(int w, int h){

    int width = p->w();
    int height = p->h();
    double w_rate = 1.00*width/w;
    double h_rate = 1.00*height/h;
    double rate = max(w_rate, h_rate);
    Fl_Image* tmp=p;

    p = p->copy(width/rate,height/rate);
    int delta_x = (w-width/rate)/2;
    move(delta_x, 0);
    delete tmp;

    return rate;
    //Fl_Image* scaledImage = new Fl_Image(p->copy(ww, hh));
}
...
//bool highlight_poly::within_polygon(Point p)const{
//    bool inside = false;
//    int num_points = points.size();
//    int j = num_points - 1;
//    for (int i = 0; i < num_points; j = i++) {
//        int xi = points[i * 2].x, yi = points[i * 2 + 1].y;
//        int xj = points[j * 2].x, yj = points[j * 2 + 1].y;
//
...
//}
    ...
    bool highlight_poly::within_polygon(Point p) const {
//    std::cout << "begin judging" << std::endl;
...
//            std::cout << "11" << std::endl;
                ...
                if (points[i].x + (p.y - points[i].y) * (points[j].x - points[i].x) / (points[j].y - points[i].y) < p.x) {
//                std::cout << "22" << std::endl;
    ...
    void highlight_poly::draw_lines() const {
//    this->set_color(FL_RED);
//    Graph_lib::Polygon::draw_lines();
        int num_points = points.size();
        int line_spacing = 5;
        // 找出多边形的边界
...
//    std::cout << min_y << " " << max_y << std::endl;
        // 在多边形内部绘制竖直线
        for (int x = min_x; x < max_x; x += line_spacing) {
            for (int y = min_y; y < max_y; y++) {
                if (within_polygon(Point(x, y ))) {
//                this->set_color(FL_YELLOW);
//                std::cout << "绘制高亮" << std::endl;
                    fl_line(x, y, x, y+1);
                }
            }
        }
    ...
    double highlight_poly::calculate_area() {
