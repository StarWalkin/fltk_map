...
struct Image : Shape {
    ...
    double change_scale(int w,int h);
...
//------------------------------------------------------------------------------
    ...
    struct highlight_poly : Polygon {    // closed sequence of non-intersecting lines
//    void add(Point p);
//    void draw_lines() const;
        highlight_poly(){}
        highlight_poly(vector<Point> init_points) {
            for (int i = 0;i < init_points.size();i ++){
                this->add(init_points[i]);
            }
            if (number_of_points() < 3) error("less than 3 points in a Polygon");
//    if (points.back() != points[0]) error("Not closed");
        }

        //used to judge whether a point is within a given polygon
        bool within_polygon(Point p)const;
        //draw lines for highlight
